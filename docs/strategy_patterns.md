...
視窗大小: 126 天（約 6 個月）
左側高點: 位於視窗前半段（前 50%）
底部位置: 在視窗 50-75% 區間（確保充足時間形成 U 型）
回檔深度: 12% ~ 33%
...
**品質特徵**:
- A 級：動能最強，成交量極度萎縮，突破最確定
- B 級：動能良好，但成交量萎縮程度不如 A 級
- C 級：滿足基本條件，但品質最弱，需特別謹慎
...
為了進一步提升系統的資金效率與風險控制能力，未來的開發將聚焦於以下幾個核心領域：

### 1. 進階資金管理 (Advanced Capital Management)

目前系統採用簡單的 FIFO (先進先出) 與固定百分比 (10%) 的資金分配方式。未來將引入更科學的數學模型：

#### Kelly Criterion (凱利公式)
*   **目標**：根據策略的勝率與賠率，計算出理論上的最佳下注比例，以最大化長期幾何成長率。
*   **標準公式**：`f* = (p * b - q) / b`
    *   `f*`: 最佳下注比例
    *   `p`: 勝率 (Win Rate)
    *   `b`: 平均盈虧比 (Avg Profit / Avg Loss)
    *   `q`: 敗率 (1-p)
*   **範例**：
    *   若勝率 60% (`p=0.6`)，平均盈虧比 2:1 (`b=2`)。
    *   `f* = (0.6 * 2 - 0.4) / 2 = 0.4`。
    *   建議配置 **40%** 資金。
*   **⚠️ 重要警告 (QuantPedia)**：
    *   **切勿使用完整凱利 (Full Kelly)**：在參數估計誤差下，Full Kelly 會導致災難性的過度下注與破產風險。
    *   **實務建議**：頂級交易員通常使用 **Half-Kelly (0.5 * f*)** 或 **Quarter-Kelly (0.25 * f*)**。
    *   以上述範例為例，實際建議倉位應為 **20% (Half)** 或 **10% (Quarter)**。

### 2. 大盤趨勢整合 (Market Trend Integration)

目前的策略主要關注個股型態。未來將結合大盤趨勢指標 (如加權指數 MA、市場寬度等) 來動態調整總曝險部位：
...
*   **盤整市場 (Choppy)**：
    ...
    *   收緊進場標準，只交易 A 級型態。
*   **空頭市場 (Downtrend)**：
    *   停止建立新多單，或僅保留極小倉位 (現金為王)。
    *   大幅收緊停損。

### 3. 嚴格風險控制規則 (Risk Control Rules)

建立系統化的風險控制層，作為所有交易的「硬限制 (Hard Limits)」：

*   **單筆風險限制 (Single Trade Risk)**：
    *   任何單筆交易的潛在虧損 (Entry - Stop) 不得超過總資金的 **2%**。
    *   這意味著如果停損幅度較大，必須相應減少倉位規模。
*   **總帳戶風險限制 (Total Portfolio Risk)**：
    *   所有持倉的潛在虧損總和不得超過總資金的 **8%**。
    *   當總風險觸及 8% 時，禁止建立新倉位，甚至需強制減倉。
    *   這能確保即使發生極端市場崩盤 (所有持倉同時觸發停損)，帳戶仍能保留 92% 的實力。

### 4. 機器學習動態調整 (ML Dynamic Adjustment)

在上述規則建立後，引入機器學習 (Machine Learning) 模型來優化決策：

*   **動態倉位評分**：訓練模型預測交易成功率，將其作為 Kelly Criterion 的輸入 `p`。
*   **特徵工程**：
    *   型態品質 (回調深度、整理天數、成交量型態)。
    *   大盤狀態 (VIX、均線乖離)。
    *   基本面數據 (營收成長、EPS)。
*   **目標**：讓 AI 學習在「高勝率時重倉，低勝率時輕倉或空手」，實現真正的動態資產配置。
